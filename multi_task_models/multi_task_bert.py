...
def preprocess_semantic_data(examples):
...
def get_encoded_datasets():
    ...
    for task_name in TASKS:
        ...
        for phase in datasets[task_name].keys():  # train, task, validation
            features_dict[task_name][phase] = datasets[task_name][phase].map(
                lambda X: convert_func_dict[task](X, label_dicts[task_name]),
                batched=True, remove_columns=datasets[task_name]['train'].column_names)
            print(
                task_name,
                phase,
                len(datasets[task_name][phase]),
