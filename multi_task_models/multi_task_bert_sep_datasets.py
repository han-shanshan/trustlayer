...
from bert_model import BertMultiHeadClassifier



model_path = "multi_task_bert_model_sep_datasets"
...
def train_multi_task_bert():
    ...
    model = BertMultiHeadClassifier("prajjwal1/bert-mini",  # "bert-base-uncased",
                                                    label_dict=labels,
                                                    id2label=id2label, 
                                                    label2id=label2id
                                                    # num_labels_dataset1=5, num_labels_dataset2=3,
...
def get_encoded_datasets(tokenizer):
    ...
    # labels = []
    # print(f"label_name_dicts = {label_name_dicts}")
    for task_name in tasks:
        idx = 0
        id2labels[task_name] = {}
        label2ids[task_name] = {}
        for label in label_name_dicts[task_name]:
            print(f"label_name_dicts[task_name] = {label_name_dicts[task_name]}")
            id2labels[task_name][idx] = label
            label2ids[task_name][label] = idx 
            idx += 1

    # data_processor.set_labels(labels=labels)
    data_processor.set_labels(labels=label_name_dicts)

    encoded_datasets = {}
    D = {}
    for task_name in tasks:
        data_processor.set_task_name(task_name)
        encoded_datasets[task_name] = datasets[task_name].map(data_processor.process_data_diff_datasets,    # preprocess_data, 
    ...
    return final_dataset, id2labels, label2ids, label_name_dicts


def inf(text):
    load_path = model_path + "/checkpoint-10955"
