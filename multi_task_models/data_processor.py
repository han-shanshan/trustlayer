...
class DataProcessor():
    ...
    def prepare_label_dict_for_a_task(self, task_name, dataset):
        if task_name is SEMANTIC_TASK_NAME: 
            return [label_name for label_name in dataset['train'].features.keys() if label_name not in ['ID', 'Tweet']]
        elif task_name is TOPIC_TASK_NAME:
            """
            DatasetDict({
    ...
    def process_topic_data_diff_datasets(self, examples):
        ...
        for label in self.labels[TOPIC_TASK_NAME]:
            labels_batch[label] = []
        ...
        labels_matrix = np.zeros((len(text), len(self.labels[TOPIC_TASK_NAME])))
        # fill numpy array
        for idx, label in enumerate(self.labels[TOPIC_TASK_NAME]):
